...
for (tentative in seq_len(nb_tent)) {
    ...
    nb_series <- sample(11:100, size = 1L)
    ...
    ws <- RJDemetra::load_workspace(normalizePath("ws_v2.xml"))
    ...
    time_cruncher_v3 <- call_cruncher(path_ws_v3, normalizePath("./jwsacruncher-3.5.1/bin/"), v3 = TRUE)

    cat("Coup de cruncher v2\n")
    time_cruncher_v2 <- call_cruncher(path_ws_v2, normalizePath("./jwsacruncher-2.2.6/bin/"), v3 = TRUE)
...
tidyr::pivot_longer(data_time, cols = -nb_series) %>%
    ggplot2::ggplot() +
