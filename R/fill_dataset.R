...
for (tentative in seq_len(nb_tent)) {
    ...
    data <- big_data[, seq_len(nb_series + 1)]
    ...
    jsap <- jws_sap(jws, 1L)
    for (k in sort(setdiff(seq_len(10000), seq_len(nb_series)), decreasing = TRUE)) {
    ...
    time_cruncher_v3 <- call_cruncher(
        path_ws = path_ws_v3,
        cruncher_bin_directory = normalizePath("~/work/software/jwsacruncher-3.6.0/bin/"),
        v3 = TRUE
    )
    ...
    time_cruncher_v2 <- call_cruncher(
        path_ws = path_ws_v2,
        cruncher_bin_directory = normalizePath("~/work/software/jwsacruncher-2.2.6/bin/"),
        v3 = FALSE
    )
...
FILE_KEY_OUT_S3 <- file.path("Compare-time-rjdverse", "ARCHIVES", paste0("time-", Sys.time(), ".csv"))
